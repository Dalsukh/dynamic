...
class GUMP
{
        ...
        foreach ($this->errors as $e) {
            ...
            switch ($e['rule']) {
                ...
                case 'validate_confirm':
                    $resp[] = "The <span class=\"$field_class\">$field</span> field needs equal to $param";
                    break;
                default:
    ...
    public function get_errors_array($convert_to_string = null)
    {
        ...
        foreach ($this->errors as $e) {
            ...
            switch ($e['rule']) {
                case 'mismatch' :
                    $resp[$field_name] = "There is no validation rule for $field";
                    break;
                case 'validate_required':
                    $resp[$field_name] = "The $field field is required";
                    break;
                case 'validate_valid_email':
                    $resp[$field_name] = "The $field field is required to be a valid email address";
                    break;
                case 'validate_max_len':
                    $resp[$field_name] = "The $field field needs to be $param or shorter in length";
                    break;
                case 'validate_min_len':
                    $resp[$field_name] = "The $field field needs to be $param or longer in length";
                    break;
                case 'validate_exact_len':
                    $resp[$field_name] = "The $field field needs to be exactly $param characters in length";
                    break;
                case 'validate_alpha':
                    $resp[$field_name] = "The $field field may only contain alpha characters(a-z)";
                    break;
                case 'validate_alpha_numeric':
                    $resp[$field_name] = "The $field field may only contain alpha-numeric characters";
                    break;
                case 'validate_alpha_dash':
                    $resp[$field_name] = "The $field field may only contain alpha characters &amp; dashes";
                    break;
                case 'validate_numeric':
                    $resp[$field_name] = "The $field field may only contain numeric characters";
                    break;
                case 'validate_integer':
                    $resp[$field_name] = "The $field field may only contain a numeric value";
                    break;
                case 'validate_boolean':
                    $resp[$field_name] = "The $field field may only contain a true or false value";
                    break;
                case 'validate_float':
                    $resp[$field_name] = "The $field field may only contain a float value";
                    break;
                case 'validate_valid_url':
                    $resp[$field_name] = "The $field field is required to be a valid URL";
                    break;
                case 'validate_url_exists':
                    $resp[$field_name] = "The $field URL does not exist";
                    break;
                case 'validate_valid_ip':
                    $resp[$field_name] = "The $field field needs to contain a valid IP address";
                    break;
                case 'validate_valid_cc':
                    $resp[$field_name] = "The $field field needs to contain a valid credit card number";
                    break;
                case 'validate_valid_name':
                    $resp[$field_name] = "The $field field needs to contain a valid human name";
                    break;
                case 'validate_contains':
                    $resp[$field_name] = "The $field field needs to contain one of these values: ".implode(', ', $param);
                    break;
                case 'validate_contains_list':
                    $resp[$field_name] = "The $field field needs to contain a value from its drop down list";
                    break;
                case 'validate_doesnt_contain_list':
                    $resp[$field_name] = "The $field field contains a value that is not accepted";
                    break;
                case 'validate_street_address':
                    $resp[$field_name] = "The $field field needs to be a valid street address";
                    break;
                case 'validate_date':
                    $resp[$field_name] = "The $field field needs to be a valid date";
                    break;
                case 'validate_min_numeric':
                    $resp[$field_name] = "The $field field needs to be a numeric value, equal to, or higher than $param";
                    break;
                case 'validate_max_numeric':
                    $resp[$field_name] = "The $field field needs to be a numeric value, equal to, or lower than $param";
                    break;
                case 'validate_min_age':
                    $resp[$field_name] = "The $field field needs to have an age greater than or equal to $param";
                    break;
                case 'validate_confirm':
                    $resp[$field_name] = "The $field field needs equal to $param";
                    break;
                //unique
                case 'validate_unique':
                    $resp[$field_name] = "The $field field already exists in ".$param[0]." table";
                    break;
                default:
                    $resp[$field_name] = "The $field field is invalid";
            }
        }
    ...
    */
    ...
    protected function validate_confirm($field, $input, $param = null)
    {
        
        if (isset($input[$field]) && $input[$field] === $input[$param]) {
            return;
        }
